...
# Pfade für Konfiguration und Persistenz
CONFIG_FILE = "/data/options.json"     # Hier speichert Home Assistant die Add‑On-Optionen
TRACKINGS_FILE = "/data/trackings.json"  # Hier werden die Sendungsdaten persistiert
...
def load_trackings():
    if os.path.exists(TRACKINGS_FILE):
        try:
            ...
            print(f"Fehler beim Laden der Sendungen: {err}")
    return {}
...
def save_trackings():
    try:
        ...
        print(f"Fehler beim Speichern der Sendungen: {err}")

# Lade die persistierten Sendungen
trackings = load_trackings()

def dhl_fetch(tracking_number):
    """
    Fragt die DHL-API für eine Trackingnummer ab.
    Die API wird über folgenden URL aufgerufen:
    https://www.dhl.de/int-verfolgen/data/search?piececode=SENDUNGSNUMMER&language=de

    Die Funktion extrahiert den 'aktuellerStatus', 'datumAktuellerStatus'
    sowie, falls vorhanden, 'fortschritt' und 'maximalFortschritt'.
    Diese Werte werden unter den englischen Schlüsseln "progress" und "maxProgress" zurückgegeben.
    """
...
def update_trackings():
    ...
    while True:
        for tn in list(trackings.keys()):
            ...
            # Falls bereits ein Paketname vorhanden ist, behalten wir ihn bei.
